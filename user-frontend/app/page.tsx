...
export default function Home() {
  ...
  const {
    ...
    isLoading,
    isFetching,
  } = useUserImages(pageNo, perPage, debouncedSearch);
  ...
  useEffect(() => {
    ...
    socket.on(
      "upload_status",
      (data: { filename: string; status: string; message: string }) => {
  ...
  return (
    <Box sx={{ display: "flex", flexDirection: "column", minHeight: "100vh" }}>
      ...
      <Box
        component="main"
        sx={{ flex: 1, p: 4, maxWidth: "1200px", mx: "auto", width: "100%" }}
      >
        <UploadForm
          ...
          >
            <Typography color="textSecondary">
              {debouncedSearch
                ...
                : "Картинок пока нет. Загрузите свою первую картинку выше!"}
            </Typography>
          </Paper>
        ) : (
          <>
        ...
        <DialogContent>
          <Box display="flex" flexDirection="column" gap={2} mt={1}>
            ...
            <TextField
              ...
              onChange={(e) =>
                setEditDialog({ ...editDialog, author: e.target.value })
              }
              fullWidth
            />
          </Box>
