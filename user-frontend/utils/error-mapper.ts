export const translateError = (errorKey: string | string[] | any): string => {
  ...
  if (typeof errorKey === 'object') {
    return translateError(errorKey.message || errorKey.error || JSON.stringify(errorKey));
  }
  ...
  const errorMap: Record<string, string> = {
    ...
    PASSWORD_IS_NOT_VALID_FORMAT: 'Пароль должен содержать минимум 1 заглавную букву, 1 строчную, 1 цифру и 1 спецсимвол.',
    ...
    'Ваш аккаунт заблокирован. Вы не можете публиковать картинки.': 'Ваш аккаунт заблокирован. Вы не можете публиковать картинки.',
  };
